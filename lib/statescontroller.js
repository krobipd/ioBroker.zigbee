...
// iterated, and right after an interview, before newDevice() is through. Such a message waits for the
// registration instead of being processed against a missing model.
const DEVICE_REGISTRATION_WAIT_MS = 30000; // upper bound for the wait, then the message is processed anyway
const MAX_WAITING_MESSAGES = 50;           // per device, a message beyond that is processed at once
...
class StatesController extends EventEmitter {
    constructor(adapter) {
        ...
        // devices newDevice() / syncDeviceState() has run for since the start, and the messages that wait for it
        this.registeredDevices = new Set();  // ieee addresses
        this.registrationGates = new Map();  // ieee -> { waiters: [resolve, ...], model, timer }
        this.stopped = false;
    ...
    async clearModelDefinitions() {
        ...
        // the models are registered again by syncDeviceState() - messages wait for it like after the start
        this.registeredDevices.clear();
    }

    // Resolves once newDevice() has run for the device of the entity - true: process the message, false: drop
    // it (the controller stops, or the device left). Synchronous up to the push, so a deviceRegistered() can
    // not fall between the check and the registration of the waiter.
    waitForDeviceRegistration(entity) {
        const device = entity.device;
        // an unsupported device is never registered (its messages hit the stash as before), a failed interview
        // never gets a newDevice(); a device still interviewing does not reach this point
        if (!entity.mapped || device.type === 'Coordinator' || device.interviewState === 'FAILED' || this.registeredDevices.has(device.ieeeAddr)) {
            return Promise.resolve(true);
        }
        if (this.stopped) return Promise.resolve(false);
        let gate = this.registrationGates.get(device.ieeeAddr);
        if (!gate) {
            gate = {
                waiters: [],
                model: entity.mapped.model, // for the log label - while pairing the device has no object yet
                timer: this.adapter.setTimeout(() => this.releaseDevice(device.ieeeAddr, 'timeout'), DEVICE_REGISTRATION_WAIT_MS),
            };
            this.registrationGates.set(device.ieeeAddr, gate);
        }
        if (gate.waiters.length >= MAX_WAITING_MESSAGES) return Promise.resolve(true);
        if (this.debugActive) this.debug(`message from '${this.devLabel(device.ieeeAddr, entity.mapped.model)}' arrived before the device was registered - waiting`);
        return new Promise(resolve => gate.waiters.push(resolve));
    }

    // Called when newDevice() / syncDeviceState() is through for the device, whether it succeeded or not.
    deviceRegistered(ieeeAddr) {
        this.registeredDevices.add(ieeeAddr);
        this.releaseDevice(ieeeAddr, 'registered');
    }

    releaseDevice(ieeeAddr, reason) {
        const gate = this.registrationGates.get(ieeeAddr);
        if (!gate) return;
        this.registrationGates.delete(ieeeAddr);
        this.adapter.clearTimeout(gate.timer);
        const count = gate.waiters.length;
        const label = this.devLabel(ieeeAddr, gate.model);
        if (reason === 'timeout') {
            // the device is not held back again - the message runs into the usual handling of a missing model
            this.registeredDevices.add(ieeeAddr);
            this.warn(`Device '${label}' was not registered within ${DEVICE_REGISTRATION_WAIT_MS / 1000} s - processing ${count} waiting message${count === 1 ? '' : 's'} anyway`);
        } else if (reason === 'registered' && count > 0) {
            this.info(`${count} message${count === 1 ? '' : 's'} from '${label}' arrived before the device was registered - processing ${count === 1 ? 'it' : 'them'} now`);
        }
        const proceed = reason === 'registered' || reason === 'timeout';
        for (const resolve of gate.waiters) resolve(proceed);
    }

    // The device left the network: waiting messages are dropped, and a device paired again waits for its
    // registration like a new one.
    forgetDevice(ieeeAddr) {
        this.registeredDevices.delete(ieeeAddr);
        this.releaseDevice(ieeeAddr, 'left');
    }
    ...
    async onZigbeeEvent(type, entity, message) {
        ...
        if (!(await this.waitForDeviceRegistration(entity))) {
            return;
        }
    ...
    async stop() {
        // messages still waiting for a device registration are dropped, nothing is registered any more
        this.stopped = true;
        for (const ieeeAddr of [...this.registrationGates.keys()]) this.releaseDevice(ieeeAddr, 'stopped');
