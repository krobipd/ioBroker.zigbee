...
class ZigbeeController extends EventEmitter {
    ...
    */
    ...
    /** herdsman's acceptJoiningDeviceHandler: false rejects the join before any interview starts. */
    acceptJoiningDevice(ieeeAddr) {
        const ieee = String(ieeeAddr).toLowerCase();
        if (this.blocklist && this.blocklist.has(ieee)) {
            this.info(`Join of '${ieee}' rejected: the device is on the blocklist`);
            return false;
        }
        return true;
    }
    ...
    async configure(options) {
        ...
        // join blocklist: herdsman asks before it interviews a joining device (rejoins included) and
        // sends a leave request instead of an interview when the device is on the list
        const blocklist = utils.parseIeeeList(options.blocklist);
        for (const entry of blocklist.invalid) {
            this.warn(`Ignoring blocklist entry '${entry}': not an IEEE address`);
        }
        this.blocklist = blocklist.ieees;
        if (this.blocklist.size > 0) {
            herdsmanSettings.acceptJoiningDeviceHandler = async ieeeAddr => this.acceptJoiningDevice(ieeeAddr);
            this.info(`Join blocklist active for ${[...this.blocklist].join(', ')}`);
        }
    ...
    async start() {
        ...
        // after the devices are registered: a message that arrives while the coordinator is busy with
        // the close would otherwise hit 'Model ... not found' for its device
        try {
            await this.closeNetwork();
            this.info('Network closed for new devices until pairing is opened');
        } catch (error) {
